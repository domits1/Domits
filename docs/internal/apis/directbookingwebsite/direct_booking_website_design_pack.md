...
- A domain another site has proven with a tenant returns `domain_taken`. A row without a tenant is not a proven claim, so a new request from another site takes it over. The takeover is a compare-and-swap on `(domain, site_id, updated_at)` as read, so a row that was provisioned or re-claimed in between answers `domain_taken` instead of being overwritten, and every status write is scoped to `(id, site_id)`; a provisioning write that finds its row gone disables the tenant it just created and answers `domain_taken`. A domain already used by another CloudFront resource (`CNAMEAlreadyExists`) is stored as `FAILED` with the reason, because only `UpdateDomainAssociation` from the owning resource can free it.
...
Error codes and statuses: `invalid_request` 400, `invalid_domain` 400, `unauthorized` 401, `forbidden` 403, `site_not_found` 404, `domain_not_found` 404, `domain_taken` 409, `domain_limit_reached` 409, `tenant_not_owned` 409, `tenant_create_failed` 502, `sync_failed` 502, `internal_error` 500.
