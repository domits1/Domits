...
export class PropertyBuilder {
    ...
    async addAmenities(amenities) {
        ...
        for (const amenity of amenities) {
            ...
            amenityArray.push(new PropertyAmenity(
                randomUUID(),
                this.property.id,
                amenity.amenityId
            ));
    ...
    addAvailability(availabilities) {
        ...
        for (const availability of availabilities) {
            ...
            availabilityArray.push(new PropertyAvailability(
                this.property.id,
    ...
    async addAvailabilityRestrictions(restriction) {
        ...
        const restrictionExists = await
            this.propertyAvailabilityRestrictionRepository.getAvailabilityRestrictionById(restriction.restriction);
        if (!restrictionExists) {
            throw new Error("Restriction not found.");
        }
        restrictionArray.push(new PropertyAvailabilityRestriction(
            randomUUID(),
            this.property.id,
    ...
    async addGeneralDetails(details) {
        ...
        for (const detail of details) {
            const detailExists =
                await this.propertyGeneralDetailRepository.getGeneralDetailById(detail.detail);
            if (!detailExists) {
    ...
    async addRules(rules) {
        ...
        for (const rule of rules) {
            const ruleExists =
                await this.propertyRuleRepository.getRuleById(rule.rule);
            if (!ruleExists) {
    ...
    addTechnicalDetails(params) {
        const technicalDetails = new PropertyTechnicalDetails(
            params
        )
