...
const WEBSITE_PUBLIC_ANALYTICS_EVENT_TYPES = new Set(["SITE_LCP_RECORDED"]);
...
export class PropertyController {
  ...
  async notifyChannexCalendarOverrideChange({ hostId, propertyId, previousOverrides, normalizedOverrides }) {
  ...
  validatePricingPayload(pricing) {
    ...
    const weekendRateRaw = pricing.weekendRate ?? pricing.weekendrate ?? pricing.weekendPrice ?? pricing.weekendprice;
      ...
      new Map(
        (Array.isArray(overrides) ? overrides : [])
          .map((entry) => {
            ...
            const nightlyPriceRaw = entry.nightlyPrice ?? entry.nightly_rate ?? entry.price;

            let nightlyPrice = null;
            if (nightlyPriceRaw !== undefined && nightlyPriceRaw !== null && nightlyPriceRaw !== "") {
              const parsedNightlyPrice = Number(nightlyPriceRaw);
              if (!Number.isFinite(parsedNightlyPrice) || parsedNightlyPrice < 2) {
                throw new Error("Calendar override nightlyPrice must be a number greater than or equal to 2.");
              }
              nightlyPrice = Math.trunc(parsedNightlyPrice);
            }

            const isAvailable = this.normalizeCalendarOverrideBoolean(entry.isAvailable, "isAvailable");
            const stopSell = this.normalizeCalendarOverrideBoolean(entry.stopSell ?? entry.stop_sell, "stopSell");
            ...
            const minStay = this.normalizeCalendarOverrideOptionalInteger(entry.minStay ?? entry.min_stay, "minStay");
            const maxStay = this.normalizeCalendarOverrideOptionalInteger(entry.maxStay ?? entry.max_stay, "maxStay");

            const priceLabsIgnoredRaw = entry.priceLabsIgnored ?? entry.pricelabs_ignored;
            const priceLabsIgnored =
              priceLabsIgnoredRaw === null || priceLabsIgnoredRaw === undefined ? null : Boolean(priceLabsIgnoredRaw);
  ...
  normalizeCalendarDateValue(value, fieldName) {
    ...
    return this.normalizeCalendarDateParts(date.getUTCFullYear(), date.getUTCMonth() + 1, date.getUTCDate(), fieldName);
  }
  ...
  normalizeLocationPayload(locationPayload) {
    ...
    const extensionFromBody =
      typeof locationPayload.houseNumberExtension === "string" ? locationPayload.houseNumberExtension.trim() : "";
  ...
  parseWebsiteDeleteReasons(payload) {
    ...
    return [...new Set(payload.map((reason) => String(reason || "").trim()).filter(Boolean))];
  }
  ...
  normalizeWebsiteBuildAnalyticsPayload(eventType, payload) {
    ...
    if (
      eventType === "WEBSITE_PREVIEW_READY" ||
  ...
  normalizeWebsiteSurfaceAnalyticsPayload(payload) {
    const surface = String(payload?.surface || "")
      .trim()
      .toLowerCase();
    const viewport = String(payload?.viewport || "")
      .trim()
      .toLowerCase();
  ...
  buildDirectBookingWebsiteSummary(site, domains = []) {
    ...
    const primaryDomain =
      normalizedDomains.find((domainEntry) => domainEntry?.isPrimary) || normalizedDomains[0] || null;
  ...
  async publishDirectBookingWebsiteForDraft({ draft, hostId, propertyId }) {
    ...
    const propertyDetails = await this.propertyService.getFullPropertyAttributesWithFullLocation(propertyId, {
  ...
  async recordHostWebsiteAnalyticsEvent({ event, propertyId, draftId, eventType, payload }) {
    ...
    const existingDraft = await this.directBookingWebsiteDraftRepository.getDraftByPropertyIdAndHostId(
      propertyId,
      hostId
    );
  ...
  async recordWebsiteAnalyticsEvent(event) {
    try {
      const eventBody = JSON.parse(event.body || "{}");
      const eventType = String(eventBody.eventType || "")
        .trim()
        .toUpperCase();
  ...
  async upsertWebsiteDraft(event) {
    try {
      ...
      const status = String(eventBody.status || "DRAFT")
        .trim()
        .toUpperCase();
      ...
      const buildCompletion = this.parseOptionalWebsiteBuildCompletionPayload(eventBody.buildCompletion);
  ...
  async getWebsiteDraftByPropertyId(event) {
    try {
      ...
      const propertyId = String(
        event.queryStringParameters?.property || event.queryStringParameters?.propertyId || ""
      ).trim();
      if (!propertyId) {
        return this.badRequest("Missing propertyId.");
      }
  ...
  async resolvePublicWebsiteSite(event) {
    try {
      ...
      return {
        ...
        headers: draftResponseHeaders,
        body: JSON.stringify(
          this.buildPublicDirectBookingWebsiteResolution(resolutionResult.site, resolutionResult.domain)
        ),
      };
    } catch (error) {
      console.error(error);
  ...
  async getPublicWebsiteRenderModel(event) {
    try {
      const siteId = cleanWebsiteText(event.queryStringParameters?.site || event.queryStringParameters?.siteId);
      const requestedDomain = this.resolveRequestedStandaloneWebsiteDomain(event);
      ...
      const propertySnapshot = await this.buildPublicPropertySnapshotForWebsiteRender(resolutionResult.site);

      return {
        statusCode: 200,
        headers: draftResponseHeaders,
        body: JSON.stringify(
          this.buildPublicDirectBookingWebsiteRenderPayload(
  ...
  async getWebsitePreviewByDraftId(event) {
    try {
      const draftId = String(event.queryStringParameters?.draft || event.queryStringParameters?.draftId || "").trim();
      ...
      const propertyDetails = await this.propertyService.getFullPropertyAttributesWithFullLocation(draft.propertyId, {
        includeCalendarAvailability: true,
      });
  ...
  async deleteWebsiteDraft(event) {
    try {
      ...
      const existingSite = await this.directBookingWebsiteSiteRepository.getSiteByPropertyIdAndHostId(
        propertyId,
        hostId
      );
  ...
  async createPropertyObject(propertyBuilder, body, userId, { skipImages = false } = {}) {
    let builder = await propertyBuilder.addBasePropertyInfo(body.property, body.propertyType.property_type, userId);
