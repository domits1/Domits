...
class ThreadRepository {
    ...
    async findThread(userId1, userId2, propertyId) {
        const client = await Database.getInstance();
        // This logic is simple and assumes hostId/guestId match userId1/userId2 in one direction
        // In reality, we should check both directions or normalized IDs
        const thread = await client
            .getRepository(UnifiedThread)
            .createQueryBuilder("thread")
            .where("(thread.hostId = :u1 AND thread.guestId = :u2) OR (thread.hostId = :u2 AND thread.guestId = :u1)", { u1: userId1, u2: userId2 })
