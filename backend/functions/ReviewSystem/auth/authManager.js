...
// Review: Accepts either a raw Cognito access token or the standard Bearer header value.
const normalizeAccessToken = (accessToken) => {
  if (!accessToken) return null;
  return String(accessToken).replace(/^Bearer\s+/i, "").trim();
};

// Review: Authenticates review requests and exposes the reviewer identity used by authorization rules.
class AuthManager {
