...
async function CalculateTotalRate (propertyId, dates) {
  ...
  const totalWithoutCleaningfee = (basepricing.roomrate * 1.1) * differenceInDays; // workaround: encapsulate pricing
  const totalWithCleaningfee = (basepricing.roomrate * 1.1 + basepricing.cleaning) * differenceInDays; // workaround: encapsulate pricing
  return {
