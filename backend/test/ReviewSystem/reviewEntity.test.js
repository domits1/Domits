...
const REVIEW_ENTITIES = [
  ...
  {
    entity: Review_Notification_Preference,
    tableName: "review_notification_preference",
    columns: ["user_id", "email_enabled", "updated_at"],
  },
  {
...
describe("Review system entities", () => {
  ...
  it.each(REVIEW_ENTITIES)("maps the primary key for $tableName", async ({ entity, tableName }) => {
    const metadata = await loadMetadata(entity);

    expect(metadata.primaryColumns.map((column) => column.databaseName)).toEqual([
      tableName === "review_notification_preference" ? "user_id" : "id",
    ]);
  });
});
