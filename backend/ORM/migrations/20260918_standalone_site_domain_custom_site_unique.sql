...
ON CONFLICT (domain)
DO UPDATE SET domain_type = EXCLUDED.domain_type, status = EXCLUDED.status, is_primary = existing.is_primary, verification_details_json = EXCLUDED.verification_details_json, last_checked_at = EXCLUDED.last_checked_at, updated_at = EXCLUDED.updated_at
