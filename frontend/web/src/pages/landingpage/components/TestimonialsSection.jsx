...
Hear from our Hosts.
    ...
    >
      {visibleReviews.map((review, i) => {
        ...
        if (i === 1) {
          positionClass = "testimonial--featured";
        } else if (i === 0) {
          positionClass = "testimonial--left";
        } else {
          positionClass = "testimonial--right";
        }

        return (
          <motion.div
            key={review.id}
            className={`testimonial ${positionClass}`}
            variants={fadeUp}
            initial="hidden"
            animate="visible"
          >
    ...
    <motion.div className="testimonials__trust" variants={fadeUp}>
      <div className="testimonials__avatars">
        {reviews.slice(0, 3).map((r) => (
          <img key={r.id} src={r.img} alt={r.author} />
        ))}
      </div>
