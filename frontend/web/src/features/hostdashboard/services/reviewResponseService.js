...
// Review: Uses local host review data when the backend review API is not configured.
    ...
    mockReviewsByHostId.set(normalizedHostId, [
      {
        ...
        overallRating: 5,
        categoryRatings: { cleanliness: 5, accuracy: 5, communication: 4, location: 5, checkIn: 4, value: 5 },
      ...
      },
      {
        ...
        overallRating: 4,
        categoryRatings: { cleanliness: 4, accuracy: 5, communication: 5, location: 4, checkIn: 5, value: 4 },
      ...
      },
      {
        ...
        overallRating: 4,
        categoryRatings: { cleanliness: 4, accuracy: 4, communication: 5, location: 4, checkIn: 5, value: 4 },
...
const upsertMockResponse = (reviewId, message, status) => {
  // Review: Mirrors the real response save/publish behavior in local mock data.
...
const sendMockReviewResponseRequest = (reviewId, method, action, payload = null) => {
  // Review: Routes mock response actions through the same verbs used by the API client.
...
export async function fetchHostReviews(hostId) {
  // Review: Loads all reviews the host is allowed to answer or compare.
  const normalizedHostId = String(hostId || "").trim();
...
async function sendReviewResponseRequest(reviewId, method, action, payload = null) {
  // Review: Sends draft, publish, edit, and delete response actions to ReviewSystem.
  if (shouldUseMockHostReviews()) {
