...
export default function PanoramaLandingTemplate({ model, onSelectTarget, activeTargetId }) {
  ...
  const gallerySlots = buildPanoramaGallerySlots(model);
  const residenceVisual = buildResidenceVisual(gallerySlots, model);
  const heroStats = Array.isArray(model.stay?.stats) ? model.stay.stats.slice(0, 4) : [];
  const featuredTrustCards = Array.isArray(model.trustCards) ? model.trustCards.slice(0, 3) : [];
  const featuredAmenities = Array.isArray(model.amenities?.featured) ? model.amenities.featured.slice(0, 8) : [];
  const featuredJourneyStops = Array.isArray(model.journeyStops) ? model.journeyStops.slice(0, 3) : [];
  const featuredPolicies = Array.isArray(model.policies?.featured) ? model.policies.featured.slice(0, 4) : [];
  const residenceMeta = [
  ...
  return (
    <article className={styles.templateSite}>
      ...
      <section id="overview" className={styles.panoramaEditorialHero} {...getScrollRevealProps(60)}>
        ...
        {showTrustCards && featuredTrustCards.length > 0 ? (
          <section
            {...getPreviewTargetMarkerProps(
              styles.panoramaTrustRail,
        ...
        <div className={styles.panoramaResidenceGrid}>
          <div className={styles.panoramaResidenceCopy}>
            ...
            {model.location?.narrative ? (
              <p className={styles.heroDescription}>{model.location.narrative}</p>
            ) : null}
