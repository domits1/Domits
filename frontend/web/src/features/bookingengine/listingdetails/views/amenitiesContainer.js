...
const AmenitiesContainer = ({ amenityIds }) => {
  ...
  const amenityObjects = amenityIds.map((a) =>
    Amenities.find((entry) => entry.id === a.amenityId)
  );

  // FEATURED (first small section)
  const featured = amenityObjects.filter((a) =>
    ALLOWED_AMENITY_CATEGORIES.includes(a?.category)
  );
