// Public property review API helpers for the listing details page.
...
// Supported review sort query values shared by mock and API requests.
const REVIEW_SORTS = new Set(["recent", "highest", "lowest"]);

// Checks every supported verified-review flag used by review payloads.
...
// Reads a review rating as a number so sorting stays stable.
const getReviewRatingValue = (review) =>
  Number(review?.overallRating ?? review?.rating ?? review?.score ?? 0) || 0;

// Reads a review date as a timestamp so recent sorting works for mock data.
...
// Checks both category score shapes returned by mock and backend reviews.
const reviewHasCategory = (review, category) =>
  review?.categoryRatings?.[category] != null || review?.categoryScores?.[category] != null;

// Calculates a one-decimal average for filtered mock review summaries.
...
// Rebuilds category averages for mock reviews after filters are applied.
...
// Builds a consistent summary object for filtered mock review data.
...
// Applies the same sort and filter behavior locally when mock review data is used.
...
export const fetchPublicPropertyReviews = async (propertyId, filters = {}) => {
  ...
  // Connect frontend controls to review API query parameters.
