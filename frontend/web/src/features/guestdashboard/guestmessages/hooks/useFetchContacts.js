    ...
    try {
      ...
      const fetchUserInfoForContacts = async (contacts) => {
        const contactDetails = await Promise.all(contacts.map(async (contact) => {
          const userInfo = await fetchUserInfo(contact.hostId);
          const latestMessage = await fetchLatestMessage(contact.hostId);
          return { ...contact, ...userInfo, latestMessage, recipientId: contact.hostId, };
        }));
