...
const HomeScreen = () => {
  ...
  const fetchPropertiesByCountry = useCallback(
    async country => {
      setLoading(true);
      ...
      try {
        ...
        const response = await propertyRepository.fetchPropertyByCountry(
          country,
          byCountryLastEvaluatedKey.id,
          byCountryLastEvaluatedKey.city,
        );
  ...
  const onFavoritePress = async id => {
    ...
    setFavorites(prevFavorites => {
      wasFavorite = prevFavorites.includes(id);
      return wasFavorite
  ...
  return (
    <>
      ...
      {(loading && originalDataSetLoaded) || favoritesLoading ? (
        ...
        <FlatList
          data={
            propertiesByCountry.length > 0 ? propertiesByCountry : properties
          }
