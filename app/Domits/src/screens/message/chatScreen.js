...
const ChatScreen = ({}) => {
  ...
  useEffect(() => {
    const subscription = client
      .graphql(graphqlOperation(subscriptions.onCreateChat))
      .subscribe({
        next: ({}) => {
  ...
  const fetchChats = async () => {
    ...
    try {
      const response = await fetch(
        'https://8pwu9lnge0.execute-api.eu-north-1.amazonaws.com/General-Messaging-Production-Read-MessagesHistory',
        {
          method: 'POST',
          headers: {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch(
        'https://qkptcbb445.execute-api.eu-north-1.amazonaws.com/ChatSendMessageFunction',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
