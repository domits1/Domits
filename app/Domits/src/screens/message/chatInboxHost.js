...
const InboxHost = ({user}) => {
  ...
  const fetchOrderedChatUsers = async () => {
    try {
      const response = await fetch(
        'https://97dww4xncg.execute-api.eu-north-1.amazonaws.com/General-Messaging-Production-Read-ContactsSorted',
        {
          method: 'POST',
    ...
    try {
      ...
      if (response.ok) {
        ...
        setChatMessages(prevChats => {
          ...
          updatedChats[recipientId] = [
            latestChat,
            ...(prevChats[recipientId] || []),
          ]
          updatedChats[recipientId].sort(
    ...
    if (userId && contacts.length > 0) {
      ...
      const fetchAllChats = async () => {
        try {
          ...
          await Promise.all(
            filteredContactIds.map(contactId => fetchLatestChat(contactId)),
          )
        } catch (error) {
