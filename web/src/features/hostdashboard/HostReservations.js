...
const HostReservations = () => {
  ...
  const pageCount = reservationDisplay
    ? Math.ceil(reservationDisplay.length / amountOfItems)
    : 0
  const indexOfLastItem = currentPage * amountOfItems
  const indexOfFirstItem = indexOfLastItem - amountOfItems
  const currentItems = reservationDisplay
    ? reservationDisplay.slice(indexOfFirstItem, indexOfLastItem)
    : []
  ...
  const handleCheckboxChange = (event, reservation) => {
    ...
    if (checked) {
      ...
      setSelectedReservations(
        selectedReservations.filter(item => item.ID !== reservation.ID),
      )
    }
  }
    ...
    switch (selectedOption) {
      case 'Booking requests':
        setReservationDisplay(
          !reversed ? pendingReservations : pendingReservations.reverse(),
        )
        break
      case 'Accepted':
        setReservationDisplay(
          !reversed ? acceptedReservations : acceptedReservations.reverse(),
        )
        break
      case 'Reserved':
        setReservationDisplay(
          !reversed ? reservedReservations : reservedReservations.reverse(),
        )
        break
      case 'Cancelled':
        setReservationDisplay(
          !reversed ? cancelledReservations : cancelledReservations.reverse(),
        )
        break
      case 'All':
    ...
    if (!userId) {
      ...
      try {
        const response = await fetch(
          'https://5ycj23b6db.execute-api.eu-north-1.amazonaws.com/default/FetchReservations',
          {
            method: 'POST',
            body: JSON.stringify({
              HostID: userId,
  ...
  const asyncUpdateReservation = async status => {
    if (
      window.confirm(
        `Do you wish to set these booking request(s) as ${status.toLowerCase()}?`,
      )
    ) {
      try {
        for (let i = 0; i < selectedReservations.length; i++) {
          ...
          const response = await fetch(
            'https://5ycj23b6db.execute-api.eu-north-1.amazonaws.com/default/UpdateReservation',
            {
              method: 'PUT',
        ...
        <section className={styles.reservationContent}>
          ...
          <div className={styles.utilBox}>
            {selectedOption === 'Booking requests' && (
              <button className={styles.refreshBtn} onClick={() => selectAll()}>
                Select all
              </button>
            )}
            <button
            ...
            {isLoading ? (
              ...
              <table>
                <thead>
                  <tr>
                    {selectedOption === 'Booking requests' && (
                      <th className="reservation-th">Select</th>
                    )}
                    <th className={styles.reservationTh}>
                      Requested on
                      <img
                        src={chevron}
                        className={styles.sort}
                        alt="sort"
                        onClick={() => sortReservations('createdAt')}
                      />
                    </th>
                    ...
                    {selectedOption === 'All' && (
                      <th className="reservation-th">
                        Status
                        <img
                          className={styles.sort}
                          src={chevron}
                          onClick={() => sortReservations('Status')}
                          alt="sort"
                        />
                      </th>
                    )}
                    <th className={styles.reservationTh}>
                      ...
                      <img
                        src={chevron}
                        className={styles.sort}
                        onClick={() => sortReservations('Price')}
                        alt="sort"
                      />
                    </th>
                  </tr>
              ...
              <div className={styles.btnBox}>
                ...
                <div className={styles.btnGroup}>
                  <button
                    ...
                    onClick={() => asyncUpdateReservation('Cancelled')}>
                    Deny
                  </button>
                  <button
                    className={styles.btnApprove}
                    onClick={() => asyncUpdateReservation('Accepted')}>
                    Approve
                  </button>
                </div>
              </div>
