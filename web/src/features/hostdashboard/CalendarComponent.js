...
function CalendarComponent({passedProp, isNew, updateDates, componentView}) {
  ...
  const [minimumAdvanceReservation, setMinimumAdvanceReservation] = useState(
    passedProp.MinimumAdvanceReservation || 0,
  )
  const [maximumStay, setMaximumStay] = useState(passedProp.MaximumStay || 0)
  const [maximumAdvanceReservation, setMaximumAdvanceReservation] = useState(
    passedProp.MaximumAdvanceReservation || 0,
  )
  const [originalMinimumStay, setOriginalMinimumStay] = useState(
    passedProp.MinimumStay || 0,
  )
  const [
    originalMinimumAdvanceReservation,
    setOriginalMinimumAdvanceReservation,
  ] = useState(passedProp.MinimumAdvanceReservation || 0)
  const [originalMaximumStay, setOriginalMaximumStay] = useState(
    passedProp.MaximumStay || 0,
  )
  const [
    originalMaximumAdvanceReservation,
    setOriginalMaximumAdvanceReservation,
  ] = useState(passedProp.MaximumAdvanceReservation || 0)
  ...
  useEffect(() => {
    ...
    passedProp.ID,
    passedProp.DateRanges,
    passedProp.MinimumStay,
    passedProp.MinimumAdvanceReservation,
    passedProp.MaximumStay,
    passedProp.MaximumAdvanceReservation,
  ])
  useEffect(() => {
    if (passedProp && passedProp.DateRanges) {
      setOriginalRanges(passedProp.DateRanges)
  ...
  const renderDates = () => {
    ...
    const safeSelectedRanges = Array.isArray(selectedRanges)
      ? selectedRanges
      : []
    ...
    for (let i = 1; i <= endDate; i++) {
      ...
      const isSelected = safeSelectedRanges.some(range =>
        isDateInRange(currentDate, range.startDate, range.endDate),
      )
      const isStartDate = safeSelectedRanges.some(range =>
        isSameDay(range.startDate, currentDate),
      )
      const isEndDate = safeSelectedRanges.some(range =>
        isSameDay(range.endDate, currentDate),
      )
      newDates.push(
        <li
          key={`active-${i}`}
  ...
  const asyncSaveDates = async () => {
    ...
    try {
      const response = await fetch(
        'https://ms26uksm37.execute-api.eu-north-1.amazonaws.com/dev/Host-Onboarding-Production-Update-AccommodationStayParameters',
        {
          method: 'PUT',
    ...
    <main className={styles.body}>
          ...
          <header>
            ...
            <nav>
              <button
                ...
                onClick={() => navigateDates('prev')}></button>
              <button
                className={styles.next}
                onClick={() => navigateDates('next')}></button>
            </nav>
          </header>
          <section>
            <ul className="days">
        ...
        <div className={styles.dateRanges}>
          ...
          <section className={styles.dateRangeSection}>
            {selectedRanges.length > 0 ? (
              selectedRanges.map((dateRange, index) => (
            ...
            <div className={styles.staying_nights}>
              <div className={styles.stayMinMaxBox}>
                <div className={styles.stayMinMaxField}>
                  <label className={styles.minMaxLabel}>
                    Minimum Stay (Days):
                  </label>
                  <div className={styles.minMaxButtons}>
                    <button
                      ...
                      onClick={() =>
                        decrementAmount(setMinimumStay, minimumStay)
                      }>
                      -
                    </button>
                    {minimumStay}
                    <button
                      className={styles.roundButton}
                      onClick={() =>
                        incrementAmount(setMinimumStay, minimumStay, 30)
                      }>
                      +
                    </button>
                  </div>
                ...
                <div className={styles.stayMinMaxField}>
                  ...
                  <div className={styles.minMaxButtons}>
                    ...
                    <button
                      className={styles.roundButton}
                      onClick={() =>
                        incrementAmount(setMaximumStay, maximumStay, 365)
                      }>
                      +
                    </button>
                  </div>
                ...
                <div className={styles.stayMinMaxField}>
                  <label className={styles.minMaxLabel}>
                    Maximum Advance Reservation (Days):
                  </label>
                  <div className={styles.minMaxButtons}>
                    <button
                      className={styles.roundButton}
      ...
      {!isNew && (
        <section className={styles.buttonBox}>
          ...
          <button className={styles.save} onClick={() => asyncSaveDates()}>
            Save
          </button>
        </section>
      )}
