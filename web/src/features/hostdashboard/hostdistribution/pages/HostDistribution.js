...
function HostDistribution() {
  ...
  const [dropdownAddChannelsVisible, setDropdownAddChannelsVisible] =
    useState(false)
  ...
  const {channelData, setChannelData, refreshChannels} =
    useManageChannels(userId)
  const [activeAddAccommodationsView, setActiveAddAccommodationsView] =
    useState(null)
  const [tempListedAccommodations, setTempListedAccommodations] = useState([])
  const [activeRemoveAccommodationsView, setActiveRemoveAccommodationsView] =
    useState(null)
  ...
  const handleAddChannel = async e => {
    ...
    const result = await handleAddChannelService(
      userId,
      selectedChannel,
      apiKey,
    )
  ...
  const handleEnableButton = channelId => {
    setChannelData(prevState => {
      const channelIndex = prevState.findIndex(
        channel => channel.id.S === channelId,
      )
  ...
  const handleAddAccommodationButton = (channelId, accommodationId) => {
    const accommodationToAdd = accommodations.find(
    ...
    const newAccommodation = {
      ...
      Availability: {
        S: accommodationToAdd.Drafted ? 'Unavailable' : 'Available',
      },
    }

    setTempListedAccommodations(prevState => {
      const existingAccommodations = prevState[channelId] || []
      const isAlreadyAdded = existingAccommodations.some(
        acc => acc.AccommodationId === newAccommodation.AccommodationId,
      )
  ...
  const handleRemoveAccommodationButton = (channelId, accommodationId) => {
    setTempListedAccommodations(prevState => {
      ...
      return {
        ...prevState,
        [channelId]: existingAccommodations.filter(
          acc => acc.AccommodationId !== accommodationId,
        ),
      }
    })
    ...
    setChannelData(prevState => {
      ...
      if (channelIndex !== -1) {
        const updatedChannels = [...prevState]
        const existingListedAccommodations =
          updatedChannels[channelIndex].ListedAccommodations.L || []

        updatedChannels[channelIndex].ListedAccommodations.L =
          existingListedAccommodations.filter(
  ...
  const renderAddAccommodationsView = id => {
    ...
    const listedAccommodations =
      channelData.find(channel => channel.id.S === channelId)
        ?.ListedAccommodations.L || []
        ...
        <table>
          ...
          <tbody>
            {accommodations
              .filter(
                accommodation =>
                  !listedAccommodationIds.includes(accommodation.ID),
              )
              .map(accommodation => (
  ...
  const renderRemoveAccommodationsView = id => {
    ...
    const combinedAccommodations = [
      ...listedAccommodations.map(acc => ({
        ...acc.M,
        id: {S: acc.M.AccommodationId},
      })),
      ...tempAccommodations.map(acc => ({
        ...acc,
        id: {S: acc.AccommodationId},
      })),
    ]
      ...
      <div className="channelManageMenuContent">
        <div className="channelManageButtonContainer">
          ...
          <div
            ...
            {activeAddAccommodationsView === channelId &&
              renderAddAccommodationsView(channelId)}
          </div>
          <button
            className="channelManageMenuButton"
            onClick={() => handleViewRemoveAccommodationsButton(channelId)}>
          ...
          <div
            ...
            {activeRemoveAccommodationsView === channelId &&
              renderRemoveAccommodationsView(channelId)}
          </div>
          <button
            className="channelManageMenuButton enabled"
            onClick={() => handleEnableButton(channelId)}>
            Enable channel
          </button>
          <button
            className="channelManageMenuButton disabled"
            onClick={() => handleDisableButton(channelId)}>
            Disable channel
          </button>
        </div>
      </div>
    ...
    <div className="containerHostDistribution">
      <div className="host-dist-header">
        ...
        <div className="addChannelButtonMenuContainer">
          ...
          <div
            className={`addChannelButtonMenu ${dropdownAddChannelsVisible ? 'visible' : ''}`}>
        ...
        <div className="channelContents">
          <div className="contentContainer-channel">
            {channelData
              ...
              .slice(
                (currentPannel - 1) * itemsPerPage,
                currentPannel * itemsPerPage,
              )
              .map((channel, index) => (
                <div
                  className="host-dist-box-container"
                  key={channel.id.S || `channel-${index}`}>
                  <div className="host-dist-box-row">
                    ...
                    <p>
                      <label className="toggle-status-switch">
                        <input
                          ...
                          onChange={() =>
                            channel.Status
                              ? handleDisableButton(channel.id.S)
                              : handleEnableButton(channel.id.S)
                          }
                        />
                    ...
                    <p className="totalListedAccommodations">
                      {channel.ListedAccommodations.L || '0'} Listed
                      Accommodations
                    </p>
          ...
          <div className="channelNavigation">
            <button
              ...
              onClick={() =>
                channelPannel(currentPannel > 1 ? currentPannel - 1 : 1)
              }
            ...
            <button
              ...
              onClick={() =>
                channelPannel(
                  currentPannel < Math.ceil(channelLength / itemsPerPage)
                    ? currentPannel + 1
                    : currentPannel,
                )
              }
