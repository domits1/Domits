...
const HostPricing = () => {
  ...
  const activeItemsPerPage =
    viewMode === 'details' ? itemsPerPageDetails : itemsPerPageTable
  ...
  useEffect(() => {
    if (accommodations.length > 0) {
      const initialRates = accommodations.map(
        acc => acc.Rent.N || acc.Rent.S || '',
      )
      const initialCleaningFees = accommodations.map(
  ...
  const handleTaxFeePopup = (details, globalIndex) => {
    const countryVAT =
      vatRates.find(rate => rate.country === details.Country?.S)?.vat || '0'
    const vatRate = parseFloat(countryVAT) / 100

    const countryTouristTax =
      touristTaxRates.find(rate => rate.country === details.Country?.S)
        ?.touristTax || '0'
    ...
    if (countryTouristTax.includes('%')) {
      ...
      touristTax = parseFloat(
        countryTouristTax.replace(/[^\d.]/g, '') || 0,
      ).toFixed(2)
    } else {
      touristTax = '0.00'
    }
    ...
    return (
      <div className="pricing-taxFee-popup-container">
        <div
          className="pricing-taxFee-popup-overlay"
    ...
    try {
      const updatedAccommodations = accommodations.map((acc, i) => {
        ...
        const cleaningFeeIncluded = extraServices.some(
          service =>
            service.S === 'Cleaning service (add service fee manually)',
        )
        ...
        if (
          cleaningFeeIncluded &&
          (editedCleaningFees[i] === undefined || editedCleaningFees[i] === '')
        ) {
      ...
      if (parsedBody && typeof parsedBody === 'object') {
        const updatedRates = parsedBody.map(
          acc => acc.Rent.N || acc.Rent.S || '',
        )
        const updatedCleaningFees = parsedBody.map(
          acc => acc.CleaningFee.N || acc.CleaningFee.S || '',
        )
      ...
      <div className="host-pricing-header">
        ...
        <div className="host-pricing-header-buttons">
          <button
            ...
            onClick={fetchAccommodationsRates}>
            Refresh
          </button>
            ...
            <div className="pricing-details-view">
              <div className="accommodation-cards">
                {currentAccommodations.map((accommodation, index) => {
                  ...
                  const extraServices =
                    accommodation.Features?.M?.ExtraServices?.L || []
                  const cleaningFeeIncluded = extraServices.some(
                    service =>
                      service.S ===
                            ...
                            {cleaningFeeIncluded ? (
                              editMode ? (
                                <input
                                  ...
                                  onChange={e =>
                                    handleCleaningFeeChange(e, globalIndex)
                                  }
                                />
                              ) : (
                                editedCleaningFees[globalIndex] ||
              ...
              <table className="pricing-table">
                ...
                <tbody>
                  {currentAccommodations.map((accommodation, index) => {
                    ...
                    return (
                      <tr key={globalIndex}>
                        <td className="pricing-table-title">
                          {accommodation.Title.S}
                        </td>
          ...
          <div className="pricing-bottom-buttons">
            <div className="pricing-navigation-buttons">
              <button
                ...
                onClick={() =>
                  pricingPannel(currentPannel > 1 ? currentPannel - 1 : 1)
                }
              ...
              <button
                ...
                onClick={() =>
                  pricingPannel(
                    currentPannel <
                      Math.ceil(accommodationsLength / activeItemsPerPage)
                      ? currentPannel + 1
                      : currentPannel,
                  )
                }
