...
function Calendar({passedProp, isNew, updateDates}) {
  ...
  const renderStaticRangeLabel = () => {
    return (
      <div>
        <button onClick={() => handleStaticRangeClick('nextWeek')}>
          Next Week
        </button>
        <button onClick={() => handleStaticRangeClick('nextMonth')}>
          Next Month
        </button>
      </div>
    )
  ...
  const asyncSetDate = async () => {
    ...
    try {
      const response = await fetch(
        'https://ms26uksm37.execute-api.eu-north-1.amazonaws.com/dev/UpdateAccommodation',
        {
          method: 'PUT',
