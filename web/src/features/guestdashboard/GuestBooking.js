...
const BookingGuestDashboard = () => {
  ...
  const handleCancelation = async booking => {
    ...
    try {
      const response = await fetch(
        'https://5vyzv89320.execute-api.eu-north-1.amazonaws.com/default/Guest-Booking-Production-Update-CancelBooking',
        {
          method: 'PUT',
          body: JSON.stringify({
            ID: booking.ID,
        ...
        <div className="bookingContent">
          <div className="bookingContentHeader">
            ...
            <div className="cancelBookingButton">
              <button className="cancelBooking" onClick={toggleBookingList}>
                Cancel Booking
              </button>
            </div>
          </div>
