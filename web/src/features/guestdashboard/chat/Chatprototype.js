...
const Chat = ({user}) => {
  ...
  const recipientEmailFromUrl = new URLSearchParams(location.search).get(
    'recipient',
  )
  ...
  const fetchChats = async recipientEmail => {
    try {
      ...
      const allReceivedChats = receivedMessages.data.listChats.items.map(
        chat => ({
          ...chat,
          isSent: false,
        }),
      ...
      const filteredUsersData = usersData.filter(userData => {
        ...
        return userChats.some(
          chat =>
            ...
            chat.recipientEmail === user.attributes.email,
        )
      })

      const sortedUsersData = filteredUsersData.sort(
        ...
        <article className="chat__message">
            ...
            <article className="chat__chatContainer" ref={chatContainerRef}>
              {chatUsers.length === 0 && (
                <article className="chat__default">
                  <p className="chat__defaultmsg">
                    You have no conversations yet. Initiate a conversation by
                    viewing listings.
                  </p>
          ...
          <nav className="chat__nav">
            ...
            <div className="chat__buttonWrapper">
              <button className="chat__button chat__button--file">
                add files
          ...
          <ul className="chat__users">
            {chatUsers.map(chatUser => (
              <li
                ...
                key={chatUser.email}
                onClick={() => handleUserClick(chatUser.email)}>
                {unreadMessages[chatUser.email] > 0 && (
                  <figure className="chat__notification">
                    {unreadMessages[chatUser.email] > 9
                      ? '9+'
                      : unreadMessages[chatUser.email]}
                  </figure>
                )}
