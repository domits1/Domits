...
const HostChatbot = () => {
  ...
  const {isRecording, handleVoiceInput, stopRecording} =
    useVoiceInput(setUserInput)
  ...
  useEffect(() => {
    const isChromiumBased = !!navigator.userAgent.match(
      /Chrome|Chromium|Edg|Brave/,
    )
  ...
  const handleAccommodationQuery = async userInput => {
    ...
    try {
      if (normalizedInput.includes('show all accommodations')) {
        ...
        const noUnderstandMessage =
          "Sorry, I didn't understand your question about accommodations."
        setMessages(prevMessages => [
          ...prevMessages,
          {
            id: messageId,
            text: noUnderstandMessage,
            sender: 'bot',
            contentType: 'text',
          },
        ])
      ...
      const errorMessage =
        "Sorry, I couldn't fetch the accommodations at this moment."
      setMessages(prevMessages => [
        ...prevMessages,
  ...
  const handleFAQQuery = userInput => {
    ...
    if (faqList.length === 0) {
      const noFaqMessage =
        "Sorry, I don't have any FAQs available at the moment."
      setMessages(prevMessages => [
        ...prevMessages,
    ...
    <div className="browser-warning">
      <div className="warning-overlay">
        <div className="warning-content">
          ...
          <p>
            This feature only works on Chromium-based browsers like Google
            Chrome.
          </p>
      ...
      <div className={`hostchatbot-container ${isChatOpen ? 'open' : ''}`}>
        ...
        <div className="hostchatbot-window">
          {messages.map(message => (
            <div
              ...
              className={`hostchatbot-message ${message.sender}`}>
          ...
          {!awaitingUserChoice && (
            <button
              className="hostchatbot-go-back-button"
        ...
        <form onSubmit={handleSubmit} className="hostchatbot-input">
          <input
            ...
            placeholder={
              pdfMode
                ? 'Cannot edit text while PDF is selected'
                : 'Type a message...'
            }
          ...
          {!isRecording ? (
            <>
              ...
              <button
                type="submit"
                disabled={loading || awaitingUserChoice || !userInput}>
                Send
              </button>
            </>
          ) : (
