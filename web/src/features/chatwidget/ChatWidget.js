...
const ChatWidget = () => {
  ...
  const loadChatHistory = async () => {
    try {
      ...
      const response = await axios.get(
        'https://clmba23cj1.execute-api.eu-north-1.amazonaws.com/default/uChatbotFetchChatHistory',
        {params},
      )
      ...
      if (response.data.messages) {
        const updatedMessages = await Promise.all(
          response.data.messages.map(async msg => {
            if (
              msg.role === 'function' &&
              Array.isArray(JSON.parse(msg.content))
            ) {
  ...
  const sendWelcomeMessage = () => {
    const welcomeMessage =
      'Hi! I am Sophia, your AI assistant. How can I assist you today?'
  ...
  const handleAIResponse = async message => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://j0ci7xg9di.execute-api.eu-north-1.amazonaws.com/default/uChatbotQueryChatGPT',
        payload,
      )
  ...
  const handleUserDecision = decision => {
    ...
    if (decision === 'human') {
      setMessages(prevMessages => [
        ...prevMessages,
        {text: 'Connecting to a human...', sender: 'system'},
      ])
      setShowConsentDecision(true)
    } else {
      setMessages(prevMessages => [
        ...prevMessages,
        {text: 'Continuing with Sophia (AI).', sender: 'system'},
      ])
    }
  }
  ...
  const handleConsentDecision = async consent => {
    ...
    if (consent === 'yes') {
      ...
      setMessages(prevMessages => [
        ...prevMessages,
        {text: 'You chose not to connect to a human.', sender: 'system'},
      ])
      setIsAIChat(true)
    }
  }
  ...
  const generateLiveChatId = async () => {
    if (!liveChatId) {
      try {
        const response = await axios.get(
          'https://5kitr1osz7.execute-api.eu-north-1.amazonaws.com/default/eChatGenerateChatID',
        )
      ...
      {isOpen && (
        <ResizableBox
          ...
          <div className="chatwidget-partner-indicator">
            {isAIChat ? (
              <span>
                Chatting with: <strong>Sophia (AI)</strong>
              </span>
            ) : (
              <div>
                <span>
                  Chatting with:{' '}
                  <strong>{employeeName + ' (Employee) ' || 'Employee'}</strong>
                </span>
                <button
                  className="chatwidget-switch-to-ai"
            ...
            <div className="chatwidget-container">
              <div className="chatwidget-messages" ref={chatMessagesRef}>
                {messages.map((message, index) => (
                  <div
                    className={`chatwidget-message ${message.sender}`}
                    key={index}>
                    ...
                    <div
                      className={`chatwidget-message-content ${message.sender}`}>
                        ...
                        {message.accommodations.map((accommodation, idx) => (
                          <div
                            ...
                            <Slider
                              ...
                              {Object.keys(accommodation.Images).map(
                                (key, index) => (
                                  <div key={index}>
                                    <img
                                      src={accommodation.Images[key]}
                                      alt={`Slide ${index + 1}`}
                                      className="chatwidget-accommodation-image"
                                    />
                                  </div>
                                ),
                              )}
                            ...
                            <div
                              className="chatwidget-accommodation-details"
                              onClick={() => handleTileClick(accommodation.ID)}
              ...
              {showConsentDecision && (
                <div className="chatwidget-decision-box">
                  <p>
                    Do you consent to saving messages for training purposes?
                  </p>
                  <button onClick={() => handleConsentDecision('yes')}>
                    Yes
                  </button>
                  <button onClick={() => handleConsentDecision('no')}>
                    No
                  </button>
                </div>
              )}
