...
import {
  touristTaxRates,
  vatRates,
} from '../../utils/CountryVATRatesAndTouristTaxes'
...
const ListingDetails = () => {
  ...
  const featureIcons = {
    ...
    Dishwasher: (
      <LocalLaundryServiceIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Basic cooking essentials': (
      <FlatwareIcon sx={{color: 'var(--primary-color)'}} />
    ),
    'Dishes and silverware': (
      <FlatwareIcon sx={{color: 'var(--primary-color)'}} />
    ),
    'Glasses and mugs': (
      <FreeBreakfastIcon sx={{color: 'var(--primary-color)'}} />
    ),
    'Cutting board and knives': (
      <FoodBankIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'First aid kit': (
      <MedicalServicesIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Books and magazines': (
      <LibraryBooksIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Universal chargers': (
      <ElectricalServicesIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Carbon monoxide detector': (
      <RadarIcon sx={{color: 'var(--primary-color)'}} />
    ),
    'Fire extinguisher': (
      <FireExtinguisherIcon sx={{color: 'var(--primary-color)'}} />
    ),
    'Lock on bedroom door': (
      <LockPersonIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Children’s books and toys': (
      <ToysIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Luggage drop-off allowed': (
      <LuggageIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Accessible-height bed': (
      <AccessibleIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Cleaning service (add service fee manually)': (
      <CleaningServicesIcon sx={{color: 'var(--primary-color)'}} />
    ),
    'Concierge service': (
      <CleaningServicesIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Grocery delivery': (
      <LocalGroceryStoreIcon sx={{color: 'var(--primary-color)'}} />
    ),
    'Airport shuttle': (
      <AirportShuttleIcon sx={{color: 'var(--primary-color)'}} />
    ),
    'Private chef': <RamenDiningIcon sx={{color: 'var(--primary-color)'}} />,
    'Personal trainer': (
      <DirectionsRunIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Energy-efficient appliances': (
      <EnergySavingsLeafIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Outdoor furniture': (
      <OutdoorGrillIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'External table': (
      <TableRestaurantIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Snorkeling equipment': (
      <ScubaDivingIcon sx={{color: 'var(--primary-color)'}} />
    ),
    ...
    'Reversing camera': (
      <ControlCameraIcon sx={{color: 'var(--primary-color)'}} />
    ),
                    ...
                    {categoryItems.map((item, index) => (
                      <li key={index} className="category-item">
                        {typeof featureIcons[item] === 'string' ? (
                          ...
                          React.isValidElement(featureIcons[item]) ? (
      ...
      try {
        ...
        if (data.body && typeof data.body === 'string') {
          ...
          if (Array.isArray(retrievedBookingDataArray)) {
            ...
            setBookedDates(
              retrievedBookingDataArray.map(booking => [
                booking.StartDate,
                booking.EndDate,
              ]),
            )
  ...
  const fetchReviewsByAccommodation = async accoId => {
    try {
      ...
      const response = await fetch(
        `https://arj6ixha2m.execute-api.eu-north-1.amazonaws.com/default/FetchReviewByAccoId`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const getHostName = host => {
    return (
      host?.Attributes?.find(attr => attr.Name === 'given_name')?.Value ||
      'Unknown Host'
    )
  }
  ...
  const handleChange = (value, setType) => {
    ...
    const total =
      setType === setAdults ? newValue + children : adults + newValue
  ...
  const setDates = (StartDate, EndDate, bookedDates) => {
    ...
    const parsedStartDate =
      today > new Date(StartDate) ? today : new Date(StartDate)
  ...
  useEffect(() => {
    const calculateTotal = () => {
      ...
      const cleaningFee = accommodation.CleaningFee
        ? parseFloat(accommodation.CleaningFee * 100)
        : 0
      const calculatedServiceFee = basePrice * 0.15

      const countryVAT =
        vatRates.find(rate => rate.country === accommodation?.Country)?.vat ||
        '0'
      const vatRate = parseFloat(countryVAT)

      const countryTouristTax =
        touristTaxRates.find(rate => rate.country === accommodation?.Country)
          ?.touristTax || '0'
      ...
      if (countryTouristTax.includes('%')) {
        ...
        calculatedTouristTaxRate = parseFloat(
          ((countryTouristTax.replace(/[^\d.]/g, '') || 0) * basePrice) / 100,
        )
      } else {
                ...
                {uniqueItems.map((feature, index) => (
                  <li key={index} className="category-item">
                    {typeof featureIcons[feature] === 'string' ? (
                      ...
                      React.isValidElement(featureIcons[feature]) ? (
                      <span className="feature-icon">
                        {featureIcons[feature]}
                      </span>
                    ) : (
  ...
  const filterAdvanceReservedDates = date => {
    ...
    minAdvanceReservation.setDate(
      today.getDate() + accommodation.MinimumAdvanceReservation,
    )
    minAdvanceReservation.setHours(0, 0, 0, 0)
    const maxAdvanceReservation = new Date()
    maxAdvanceReservation.setDate(
      today.getDate() + accommodation.MaximumAdvanceReservation,
    )
    ...
    if (accommodation.MaximumAdvanceReservation === 0) {
      ...
      return (
        selectedDate >= minAdvanceReservation &&
        selectedDate <= maxAdvanceReservation
      )
    }
  }
    ...
    const mergeDateRanges = ranges => {
      ...
      for (let i = 1; i < sortedRanges.length; i++) {
        ...
        if (
          ...
          currentEnd.getTime() + 86400000 === nextStart.getTime()
        ) {
          currentRange.endDate = new Date(
            Math.max(new Date(currentRange.endDate), new Date(range.endDate)),
          )
        } else {
  ...
  const combinedCheckInDateFilter = date => {
    ...
    return (
      !isInThePast && !isOutsideMergedRanges && !isBooked && isAdvanceReserved
    )
  }
            ...
            <div>
              ...
              <div>
                ...
                <p className="houseRulesDetails">
                  {accommodation.AllowParties
                    ? 'Parties Allowed'
                    : 'No Parties'}{' '}
                  • {accommodation.AllowPets ? 'Pets Allowed' : 'No Pets'} •{' '}
                  {accommodation.AllowSmoking
                    ? 'Smoking Allowed'
                    : 'No Smoking'}
                </p>
                <div className="checkInCheckOut">
                  <div className="checkIn">
                    Check-in From: {accommodation.CheckIn?.From} To:{' '}
                    {accommodation.CheckIn?.Til}
                  </div>
                  <div className="checkOut">
                    Check-out From: {accommodation.CheckOut?.From} To:{' '}
                    {accommodation.CheckOut?.Til}
                  </div>
                </div>
              </div>
                  ...
                  {host && (
                    <div className="host-card">
                      <section className="card-top">
                        ...
                        <p>
                          Joined on:{' '}
                          {dateFormatterDD_MM_YYYY(host.UserCreateDate)}
                        </p>
                      </section>
        ...
        {accommodation && (
          <aside className="detailSummary">
            <div className="booking-info">
              <p>Booking Info</p>
              <div className="booking-info-tooltip">
                Stay for {accommodation.MinimumStay} –{' '}
                {accommodation.MaximumStay} nights.
                <br />
                Book {accommodation.MinimumAdvanceReservation} –{' '}
                {accommodation.MaximumAdvanceReservation} days in advance.
              </div>
            </div>
            <div className="summary-section">
              <h2 className="price-per-night">
                €{accommodation.Rent}{' '}
                {accommodation.Type === 'Boat' ? 'Day' : 'Night'}
              </h2>
              ...
              <div
                ...
                <div
                  className="summaryBlock dateInput"
                  style={{
                    flex: '1',
                    position: 'relative',
                    marginRight: '10px',
                    ...
                    minDate={
                      checkIn
                        ? accommodation.MinimumStay > 0
                          ? new Date(
                              checkIn.getTime() +
                                accommodation.MinimumStay * 24 * 60 * 60 * 1000,
                            )
                    ...
                    maxDate={
                      checkIn
                        ? accommodation.MaximumStay > 0
                          ? new Date(
                              checkIn.getTime() +
                                accommodation.MaximumStay * 24 * 60 * 60 * 1000,
                            )
              ...
              {isDemo ? (
                <p className="disclaimer">
                  *This is a demo post and not bookable
                </p>
              ) : (
                  ...
                  <div className="price-item">
                    <p>
                      {(new Date(checkOut) - new Date(checkIn)) /
                        (1000 * 60 * 60 * 24)}{' '}
                      nights x €{accommodation.Rent} a night
                    </p>
                    <p>
                      €
                      {((new Date(checkOut) - new Date(checkIn)) /
