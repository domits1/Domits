...
function Header({setSearchResults, setLoading}) {
  ...
  const checkAuthentication = async () => {
    try {
      ...
      setCurrentView(
        user.attributes['custom:group'] === 'Host' ? 'host' : 'guest',
      )
    } catch (error) {
      setIsLoggedIn(false)
    }
      ...
      <nav className={`header-nav ${isBarActive ? 'hide-other-content' : ''}`}>
        ...
        <div className="header-right">
          ...
          <div className="header-personal-menu-dropdown">
            ...
            <div
              className={`header-personal-menu-dropdown-content ${dropdownVisible ? 'show' : ''}`}>
              {isLoggedIn ? (
                renderDropdownMenu()
              ) : (
                <>
                  <button
                    ...
                    className="header-dropdown-login-button">
                    Login
                    <img src={loginArrow} alt="Login Arrow" />
                  </button>
                  <button
                    onClick={() => navigate('/register')}
                    className="header-dropdown-register-button">
                    Register
                  </button>
                </>
              )}
