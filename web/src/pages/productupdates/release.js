    ...
    <div className="release">
      <div className="release__header-container">
        <div className="release__header">
          ...
          <button
            ...
            type="button"
            onClick={() => navigate('/releaseTwo')}>
            Read More
          </button>
        </div>
        <img
          className="release__image-header"
          src={release}
          alt="Release"
          width={300}
        />
      ...
      <div className="release__notes">
        {releaseNotes.map((note, index) => (
          <div
            className={`release__item ${note.isOpen ? 'release__item--expanded' : ''}`}
            onClick={() => toggleOpen(index)}
            key={index}>
            <img
              className="release__item-image"
              src={note.image}
              alt={note.releaseNote}
            />
            <h4 className="release__item-title">{note.releaseNote}</h4>
            <div className="release__details">
              {note.details.split('\n').map((line, index) => (
                <p className="release__item-text" key={index}>
                  {line}
                </p>
              ))}
            </div>
