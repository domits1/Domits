    ...
    async function fetchData() {
      try {
        const response = await axios.get(
          'https://kcmfezurd1.execute-api.eu-north-1.amazonaws.com/dev/StripeID',
        )
          ...
          {Array.isArray(payments) ? (
            payments.map(payment => (
              <li key={payment.id}>
                ...
                <br />
                <strong>Cancel URL:</strong> {payment.cancelUrl}
                <br />
                <strong>Currency:</strong> {payment.currency}
                <br />
                <strong>Product Name:</strong> {payment.productName}
                <br />
                <strong>Success URL:</strong> {payment.successUrl}
                <br />
              </li>
            ))
          ) : (
