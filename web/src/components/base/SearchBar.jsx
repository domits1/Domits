...
export const SearchBar = ({setSearchResults, setLoading, toggleBar}) => {
  ...
  const GuestCounter = React.memo(
    ({label, value, onIncrement, onDecrement, description}) => {
      return (
  ...
  const getCountryCode = countryName => {
    ...
    let country = countries.all.find(
    ...
    if (!country) {
      country = countries.all.find(
        c =>
          c.name.toUpperCase() ===
          countryName.toUpperCase().replace(/^THE\s+/, ''),
      )
    }
  ...
  return (
    <>
      {error && (
        <div className="Search-error-message" onClick={handleClick}>
          {error} <FaTimesCircle />
        </div>
      )}
      <div className="bar-container">
        ...
        {(showSearchBar || !isMobile) && (
          <div
            className={`SearchBarContainer ${isBarActive ? 'active' : 'inactive'}`}>
            <div className="Search-bar">
                                ...
                                <div
                                  ...
                                  {...getSuggestionItemProps(suggestion, {
                                    style: {
                                      ...
                                      cursor: 'pointer',
                                      transition:
                                        'background-color 0.2s ease, transform 0.2s ease, border-radius 0.2s ease',
                                      fontSize: '1rem',
                                      color: '#000',
              ...
              <div className="searchInputContainer">
                <Select
                  ...
                  isClearable={true}
                  placeholder={
                    <span className="searchTitle">Accommodation</span>
                  }
                  styles={{
                    control: provided => {
              ...
              <div
                className={`Search-button-section ${showGuestDropdown ? 'active' : ''}`}
                onClick={toggleGuestDropdown}>
                <p
                  className={`searchTitleGuest ${totalGuests > 0 ? 'hidden' : ''}`}>
                  Guests • Rooms
                </p>
                {totalGuests > 0 && (
                  <button
                ...
                <div
                  className={`Search-guest-dropdown ${showGuestDropdown ? 'active' : ''}`}
                  ...
                  <GuestCounter
                    ...
                    value={adults}
                    onIncrement={() =>
                      setAdults(adults < 13 ? adults + 1 : adults)
                    }
                    onDecrement={() =>
