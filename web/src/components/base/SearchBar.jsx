...
export const SearchBar = ({setSearchResults, setLoading, toggleBar, sourceLanguage, targetLanguage}) => {
    ...
    const [translatedTexts, setTranslatedTexts] = useState({
        ...
        searchAndFilter: "Search & Filter Accommodations",
        loadingText: "Loading",
        adultsLabel: "Adults",
        adultsDescription: "Ages 16 or above",
        childrenLabel: "Children",
        childrenDescription: "Ages 2–16",
        infantsLabel: "Infants",
        infantsDescription: "Ages 0–2",
        petsLabel: "Pets",
        petsDescription: "Normal sized pets",
        roomsLabel: "Rooms",
        roomsDescription: "Amount of rooms in the Accommodation",
        searchText: "Search",
    ...
    useEffect(() => {
        const translateTexts = async () => {
            const translations = {};
            for (const [key, text] of Object.entries(translatedTexts)) {
                translations[key] = await fetchTranslation(text, sourceLanguage, targetLanguage);
            }
                        ...
                        <div className="Search-bar">
                                            ...
                                            <div className="autocomplete-container"
                                                ...
                                                <input
                                                    {...getInputProps({
                                                        ...
                                                        type: 'search',
                                                        placeholder: translatedTexts.searchDestination,
                            ...
                            <div className="searchInputContainer">
                                <Select
                                    ...
                                    options={[
                                        {value: 'Apartment', label: <><FaBuilding/> {translatedTexts.apartment}</>},
                                        {value: 'House', label: <><FaHome/> {translatedTexts.house}</>},
                                        {value: 'Villa', label: <><FaUmbrellaBeach/> {translatedTexts.villa}</>},
                                        {value: 'Boat', label: <><FaShip/> {translatedTexts.boat}</>},
                                        {value: 'Camper', label: <><FaCaravan/> {translatedTexts.camper}</>},
                                        {value: 'Cottage', label: <><FaTree/> {translatedTexts.cottage}</>},
                                        {value: 'Hotel', label: <><FaHotel/> {translatedTexts.hotel}</>},
                                    ]}
                                    isSearchable={false}
                                    isClearable={true}
                            ...
                            <div className={`Search-button-section ${showGuestDropdown ? 'active' : ''}`}
                                ...
                                <p className={`searchTitleGuest ${totalGuests > 0 ? 'hidden' : ''}`}>{translatedTexts.guestsRooms}</p>
                                {totalGuests > 0 && (
                                    <button
                                ...
                                <div className={`Search-guest-dropdown ${showGuestDropdown ? 'active' : ''}`}
                                    ...
                                    <GuestCounter
                                        label={<><FaUser/> {translatedTexts.adultsLabel}</>}
                                        description={translatedTexts.adultsDescription}
                                        value={adults}
                                        onIncrement={() => setAdults(adults < 13 ? adults + 1 : adults)}
                                        onDecrement={() => setAdults(adults > 0 ? adults - 1 : adults)}
                                    />
                                    <GuestCounter
                                        label={<><FaChild/> {translatedTexts.childrenLabel}</>}
                                        description={translatedTexts.childrenDescription}
                                        value={children}
                                        onIncrement={() => incrementGuests(children, setChildren)}
                                        onDecrement={() => setChildren(children > 0 ? children - 1 : children)}
                                    />
                                    <GuestCounter
                                        label={<><FaBaby/> {translatedTexts.infantsLabel}</>}
                                        description={translatedTexts.infantsDescription}
                                        value={infants}
                                        onIncrement={() => incrementGuests(infants, setInfants)}
                                        onDecrement={() => setInfants(infants > 0 ? infants - 1 : infants)}
                                    />
                                    <GuestCounter
                                        label={<><FaPaw/> {translatedTexts.petsLabel}</>}
                                        description={translatedTexts.petsDescription}
                                        value={pets}
                                        onIncrement={() => incrementGuests(pets, setPets)}
                                        onDecrement={() => setPets(pets > 0 ? pets - 1 : pets)}
                                    />
                                    <GuestCounter
                                        label={<><FaDoorClosed/> {translatedTexts.roomsLabel}</>}
                                        description={translatedTexts.roomsDescription}
                                        value={rooms}
