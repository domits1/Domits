...
import '../../base/ConfirmRegister.css'
...
function DigitInputs({ amount, inputRef }) {
    ...
    const inputActionMap = {
        ...
        'PREVIOUS': (digitIndex) => setTimeout(() => { inputRef.current[digitIndex === 0 ? 0 : digitIndex - 1].select() }, 10),
        'NEXT': (digitIndex) => setTimeout(() => { inputRef.current[digitIndex === amount - 1 ? amount - 1 : digitIndex + 1].select() }, 10),
        'NONE': () => {}
    };
