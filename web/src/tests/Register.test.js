...
describe.skip('Register', () => {
  it('should navigate to confirm-email when registration is successful', async () => {
    ...
    render(
      <FlowContext.Provider
        value={{flowState: {isHost: false}, setFlowState: jest.fn()}}>
        <Register />
      </FlowContext.Provider>,
    )
  ...
  it('should display error message when username is less than 4 characters', () => {
    ...
    fireEvent.click(screen.getByText(/Sign Up/i))
    expect(
      screen.getByText('Username must be at least 4 characters long.'),
    ).toBeInTheDocument()
  })
  ...
  it('should toggle isHost state when handleHostChange is called', () => {
    ...
    render(
      <FlowContext.Provider
        value={{flowState: {isHost: false}, setFlowState: mockSetFlowState}}>
        <Register />
      </FlowContext.Provider>,
    )
  ...
  it('should generate a random username during form submission', async () => {
    ...
    const generateRandomUsernameSpy = jest.spyOn(
      Register.prototype,
