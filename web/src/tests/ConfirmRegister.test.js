...
describe.skip('ConfirmEmail', () => {
  ...
  test('renders the form initially', () => {
    ...
    expect(
      screen.getByText('Enter 6 digit code sent to your email'),
    ).toBeInTheDocument()
    expect(
  ...
  test('handles error during confirmation', async () => {
    ...
    expect(
      screen.getByText(
        'Invalid verification code or your account may already be confirmed. Please try to log in.',
      ),
    ).toBeInTheDocument()
  })
  ...
  test('handles error during resending verification code', async () => {
    ...
    expect(
      screen.getByText('Failed to resend code, please try again later.'),
    ).toBeInTheDocument()
  })
})
